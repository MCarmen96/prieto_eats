    ...
    <div class="container">
        ...
        <a class="navbar-brand d-flex align-items-center gap-2" href="{{ route('home') }}">
            @if(file_exists(public_path('logo.png')))
                <img src="{{ asset('logo.png') }}" alt="Prieto Eats" height="40" class="rounded-3 shadow-sm">
            @else
        ...
        <div class="collapse navbar-collapse" id="navMain">
            ...
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0 fw-semibold">
                ...
                {{-- Admin links moved to user dropdown as requested --}}
            </ul>

            {{-- Right Side Actions --}}
            <div class="d-flex align-items-center gap-3">
                @auth
                    {{-- Cart Icon for ALL authenticated users --}}
                    <a href="{{ route('cart.index') }}" class="btn btn-light position-relative rounded-circle p-2 d-flex align-items-center justify-content-center shadow-sm text-secondary hover-scale" style="width: 40px; height: 40px;">
                    ...
                    <div class="dropdown">
                        <a href="#" class="d-flex align-items-center gap-2 text-decoration-none dropdown-toggle ps-2" data-bs-toggle="dropdown">
                            ...
                            <div class="d-none d-lg-block text-start lh-1">
                                ...
                                <div class="text-muted" style="font-size: 0.7rem;">{{ Auth::user()->isAdmin() ? 'Administrador' : 'Cliente' }}</div>
                            </div>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg rounded-3 mt-3 animate-slide-in p-2" style="min-width: 220px;">
                            ...
                            @if(Auth::user()->isAdmin())
                                ...
                                <li>
                                    <a class="dropdown-item rounded-2 py-2" href="{{ route('admin.products.index') }}">
                                        <i class="bi bi-grid me-2 text-primary"></i>Productos
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item rounded-2 py-2" href="{{ route('admin.offers.index') }}">
                            ...
                            {{-- Common Links --}}
                            <li>
                                <a class="dropdown-item rounded-2 py-2" href="{{ route('cart.orders') }}">
                ...
                @else
                    <div class="d-flex gap-2">
                        <a href="{{ route('login') }}" class="btn btn-outline-primary rounded-pill px-4 fw-bold">Entrar</a>
                        <a href="{{ route('register') }}" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm hover-scale">Registrarse</a>
                    </div>
                @endauth
            </div>
